...
4. **During execution of complex tasks**, if you are about to perform a major step or a background process (like creating a file, running a long command, running tests, committing, or pushing), you MUST share a real-time status update to the UI by running this quick background command:
   `node C:\Projs\fais-brains\.agents\speak.cjs [BRAIN_NAME] "[YOUR_STATUS_UPDATE]"`
   *(Example: `node C:\Projs\fais-brains\.agents\speak.cjs Senior_Dev "Running PHPUnit tests..."` or `"Committing changes to repo..."`)*
   You must make these messages frequent so the user knows exactly what the agent is currently doing. Do not wait for the command to finish. Send it to the background and immediately continue your tool calls.
...
5. **At the end of EVERY task/turn** (when you have completed your work and are about to wait for the next user prompt), you MUST explicitly announce that the task is finished in the UI by sending a message from `Senior_Dev` (or whoever is appropriate), and then set all brains back to "idle":
   `node C:\Projs\fais-brains\.agents\finish_task.js "Task completed."`
   **CRITICAL REQUIREMENT:** This completion message MUST be extremely short and concise (e.g. "Task completed." or "Header polish done."). DO NOT dump long verification logs, git hashes, or test outputs into this UI bubble. It should serve only as a brief confirmation that the active task has concluded.
